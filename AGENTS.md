...
- `lib/domain/card-rules.json` and `lib/domain/card-names.json` (other languages' card names → English) are generated: never edit them by hand. It covers only the current Standard format, and the parser sorts cards with it, so rotated cards land in `unknown` on purpose. When a new set releases, or a current card lands in `unknown` from `gameLog.getViewerCards`/`getOpponentCards`, rerun `node scripts/build-card-rules.ts` and commit the result.
